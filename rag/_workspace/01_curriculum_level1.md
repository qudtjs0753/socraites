...
embeddings = HuggingFaceEmbeddings(
    ...
    cache_folder=os.getenv("EMBED_CACHE_DIR", "./models"),  # 수동 설치 경로 (python 실행 디렉토리 기준)
)
...
emb = [np.array(v) for v in embeddings.embed_documents(texts)]
...
| 환경변수 관리 | `.env` + `python-dotenv` | `LLM_API_KEY`, `EMBED_CACHE_DIR` 관리에 그대로 적용 |
...
- "이 코드를 컨테이너화한다면 무엇이 환경변수로 빠져야 하는가?" (`LLM_BASE_URL`, `LLM_API_KEY`, `EMBED_CACHE_DIR`, `CHROMA_PERSIST_DIR`)
...
- [ ] **HuggingFace 임베딩 모델 수동 설치** (폐쇄망 — 외부 PC에서 다운로드 후 전달)
  ```bash
  # 외부 PC에서 실행 (인터넷 가능 환경)
  pip install huggingface_hub
  python -c "
  from huggingface_hub import snapshot_download
  snapshot_download('BAAI/bge-m3', local_dir='./models/BAAI/bge-m3')
  "
  # ./models/ 디렉토리를 압축하여 폐쇄망으로 전달
  ```
- [ ] **`.env` 파일 작성**
  ```
...
- GPU  (BGE-M3 CPU 추론으로 충분, Level 4 GPU 노드 풀은 나중에)
